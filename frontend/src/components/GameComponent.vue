      ...
      <aside class="players-panel" :class="{ 'panel-open': isPlayersPanelOpen }">
        ...
        <div class="players-list">
          ...
          >
            ...
            <div class="player-info">
              ...
              <div class="player-role">{{ player.character || 'Без персонажа' }}</div>
            </div>
        ...
        <div class="current-player-info">
          ...
          <div class="current-player">
            <span class="player-color-dot" :style="{ background: currentPlayer.color }"></span>
          ...
          <div class="chat-input-area">
            ...
            <div class="input-container">
              <div class="input-wrapper" :class="{ 'input-focused': isInputFocused }">
                <div class="input-player-badge" :style="{ background: currentPlayer.color }">
...
export default {
  ...
  data() {
    return {
      // Мир
      worldName: 'ЭХО АНДРОМЕДЫ',
      worldSetting: '2387 год. Экипаж корабля «Странник» исследует аномалию в секторе Андромеды. Колонисты сообщают о видениях и исчезновениях. Вы должны раскрыть тайну древней цивилизации.',
      mainGoal: 'Найти источник сигнала древней цивилизации и предотвратить катастрофу.',

      // Игроки
      players: [
        { id: 'alex', name: 'Алекс', character: 'Капитан', avatar: 'А', color: '#5B8CBE' },
        { id: 'maria', name: 'Мария', character: 'Пилот', avatar: 'М', color: '#6B9E7A' },
        { id: 'dmitry', name: 'Дмитрий', character: 'Хакер', avatar: 'Д', color: '#B57B5C' }
      ],

      // Текущий игрок
      currentPlayerId: 'alex',
  ...
  computed: {
    currentPlayer() {
      return this.players.find(p => p.id === this.currentPlayerId) || this.players[0];
    },
    ...
    inputPlaceholder() {
      ...
      return `${this.currentPlayer.name}, опишите действие...`;
    }
  },
  ...
  mounted() {
    setTimeout(() => {
      this.addAIMessage(
          'Добро пожаловать на борт «Странника». Датчики зафиксировали аномалию в 6 часах полёта. Капитан, какие будут приказы?'
      );
    }, 500);
  ...
  methods: {
    ...
    handleResize() {
      // Закрываем панель на десктопе при изменении размера
    ...
    sendMessage() {
      ...
      this.addPlayerMessage(this.currentPlayer, text);
      this.newMessage = '';
    ...
    addPlayerMessage(player, text) {
      this.messages.push({
        ...
        type: 'player',
        author: player.character ? `${player.name} (${player.character})` : player.name,
        text: text,
        color: player.color,
        timestamp: Date.now()
      });
    ...
    respondAsAI(playerMessage) {
      ...
      setTimeout(() => {
        ...
        if (lowerMsg.includes('исслед') || lowerMsg.includes('осмотр') || lowerMsg.includes('сканир')) {
          response = 'Сенсоры обнаружили слабый энергетический сигнал в 2 парсеках. Похоже на технологию древних. Рекомендую осторожное сближение.';
        } else if (lowerMsg.includes('оружие') || lowerMsg.includes('атак') || lowerMsg.includes('щит')) {
          response = 'Оружейные системы активированы. Но советую не провоцировать неизвестный объект. Мы не знаем его возможностей.';
        } else if (lowerMsg.includes('связ') || lowerMsg.includes('контакт') || lowerMsg.includes('сигнал')) {
          response = 'Попытка установить связь... Ответа нет. Но кто-то определённо слушает. Продолжайте попытки или действуйте иначе.';
        } else if (lowerMsg.includes('привет') || lowerMsg.includes('здравств')) {
          response = 'Приветствую. Системы корабля в норме. Жду дальнейших указаний.';
        } else {
          response = 'Принято. Продолжаю наблюдение. Что планируете делать дальше?';
        }
    ...
    insertDiceRoll(dice) {
      ...
      const message = `Бросаю d${dice}: ${roll}`;
...
.menu-toggle:hover {
  background: rgba(100, 110, 150, 0.4);
  border-color: #7B6FB0;
}
...
.input-wrapper.input-focused {
  border-color: #7B6FB0;
  box-shadow: 0 0 16px rgba(123, 111, 176, 0.2);
}
...
.send-btn.has-message {
  ...
  border-color: #7B6FB0;
  color: #E8F0FF;
  box-shadow: 0 4px 16px rgba(91, 110, 154, 0.3);
}
...
.send-btn.has-message:hover .send-icon {
...
@media (max-width: 360px) {
  ...
  .setting-header span:first-child,
  .setting-header span:last-child,
